...
public class DataMapViewController implements Initializable {
  ...
  @FXML private AnchorPane mapPane; //The pane that holds the following panes and makes the screen darker
  ...
  @FXML private JFXButton selectNode1Button; //Button to select the first node when creating an edge

  @FXML private JFXButton selectNode2Button; //Button to select the second node when creating a node
  ...
  private void drawNode(Node node) { //draws the given node on the map
    ...
    double yPos = (node.getYCoord() * heightRatio - buttonSize / 2.0); //points to the center of the button
    ...
    button.setOnAction( //when a user clicks a node:
        action -> {
          ...
          if (!edgeSelection(node)) {
            ...
            displayData();
          } //This makes sure that the action of selecting 2 nodes for an edge on a map differs from selecting and displaying a node
          // If the user is selecting 2 nodes for an edge, the modify pane as well as the nodes info should not be visible
        });
    ...
    try {
      ...
      line.setOnMouseClicked(
          mouseEvent -> { //when a user clicks on a line:
            ...
            displayEdgeData(); //The edge pain that displays the edges information should be displayed
          });
  ...
  @FXML
  private void cancelNodePane() { //this is called when the red x button is clicked (top right corner of the pane)
    resetNodePane(); //resets the values in the node pane
    modifyNodePane.setVisible(false); //makes the pane not visible
  }

  @FXML
  private void cancelEdgePane() { //this is called when the red x button is clicked (top right corner of the pane)
  ...
  private void deleteData() {
    ...
    resetNodePane();
    for (Edge edge : edgeFactory.getAllEdgesConnectedToNode(node.getId())) { //for every edge connected to the selected node
      for (int i = 0; i < mapPane.getChildren().size(); i++) { //for child in the pane
        ...
        if (children instanceof Line && children.getId().equals(edge.getId())) { //if the children is of type line and equals an edge that is connected to the selected node
  ...
  private void addNodePane() throws ValidationException {
    ...
    deleteButton.setVisible(false); //modify and delete button are not visible, these buttons are only visible when the user selects on a node on the map
    addNodeButton.setVisible(true); //the add button should be visible
    addNodeButton.setDisable(true); //the add button should only be enabled when all fields are occupied
  ...
  private void addNode() throws Exception {
    ...
    try { // is the input valid?
      if (newNode == null) { // is the ID available?
        newNode =
            new Node(
                ID, xCoordinate, yCoordinate, building, longName, shortName, nodeType, floorNumber); //creates a new node
  ...
  private void modifyData() throws ValidationException {
    ...
    try { // is the input correct?
      ...
      nodeFactory.update(node); //finds the node by the ID (this is why the ID is uneditable) and updates the node
      for (Edge edge : edgeFactory.getAllEdgesConnectedToNode(node.getId())) { //for all of the edges connected to the node
        for (int i = 0; i < mapPane.getChildren().size(); i++) { //for all of the children in the pane
          javafx.scene.Node children = mapPane.getChildren().get(i);
          if (children instanceof Line && children.getId().equals(edge.getId())) { //if a child is an instance of a line and the ID matches one of the ID that is connected to the node
            Line line = (Line) children;
            if (edge.getNode1().equals(node.getId())) { //if node one then it is a starting coordinate
      ...
      nodeButton.setLayoutY(yCoordinate * heightRatio - 3); //if the position of the node is changes, then the edges are updated as well
      resetNodePane();
      modifyNodePane.setVisible(false);
  ...
  private void addEdgePane() throws ValidationException {
    ...
    deleteEdgeButton.setVisible(false); //modify and delete should only be displayed when selecting an edge on the map
  ...
  private void displayData() { // when a node is clicked, the node's attributes will be displayed
    ...
    floorInput.setText("" + node.getFloor()); //converts the stored data into strings and displays it
  }

  private boolean edgeSelection(Node node) { //this function helps differentiate when a user is selecting a node or selecting two nodes for creating an edge
    if (selectNode1) {
      node1 = node;
      selectNode1 = false;
      addEdgePane.setVisible(true);
  ...
  @FXML
  void selectNode1(ActionEvent event) { //called when the "Select Node 1" button is pressed (when the user is adding an edge)
    addEdgePane.setVisible(false); //displays the map
    selectNode1 = true;
    node1 = null;
    displayNodePaneButton.setVisible(false);
    displayEdgePaneButton.setVisible(false); //sets the two buttons in the bottom right corner to be invisible
    if (!selectNode2Button.getText().equals("Select Node 2")) { //checks the other button (Select Node 2), if populated, the add button is activated
      addEdgeButton.setDisable(false);
    }
  }

  @FXML
  void selectNode2(ActionEvent event) { //called when the "Select Node 1" button is pressed (when the user is adding an edge)
    addEdgePane.setVisible(false); //displays the map
    selectNode2 = true;
    node2 = null;
    displayNodePaneButton.setVisible(false);
    displayEdgePaneButton.setVisible(false); //sets the two buttons in the bottom right corner to be invisible
    if (!selectNode1Button.getText().equals("Select Node 1")) { //checks the other button, if populated, the add button is activated
      addEdgeButton.setDisable(false);
    }
  }
  ...
  private void displayEdgeData() {
    ...
    selectNode2Button.setText(edge.getNode2()); //Sets the text of the two buttons to the ID of the node they correspond to
  ...
  public void modifyEdge() throws ValidationException {
    ...
    //the reason we delete then add for modifying is because the edgeFactory needs the ID to NOT change in order to update the values
    //however, the ID of an edge is dependent on the two nodes it is connected to, and if the admin wants to change the connected nodes, the ID has to change as well
    addEdgePane.setVisible(false);
    resetEdgeAddPane();
  }
  ...
  public void validateNodeText(KeyEvent keyEvent) {
    if (!xCoorInput.getText().isEmpty()
      ...
      modifyButton.setDisable(true);
      addNodeButton.setDisable(true); //if the text is added then deleted, this checks for that condition
    }
  }
  ...
  private void resetNodePane() {
    ...
    addNodeButton.setVisible(false); //add button is not visible, only accessible by pressing the add node button on the bottom right of the screen
