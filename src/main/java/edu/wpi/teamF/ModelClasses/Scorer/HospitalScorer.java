...
public class HospitalScorer implements  Scorer {
    ...
    private boolean sameHospital(String building1,String building2) {
        return ("Faulkner".equals(building1) && "Faulkner".equals(building2)) || (!"Faulkner".equals(building1) && !"Faulkner".equals(building2));
    }
    public HospitalScorer(Map<String,Node> nodeMap) {
        ...
        for (Node node : nodeMap.values()) {
            if (sameHospital(node.getBuilding(),"Faulkner")) {
                ...
                for (Edge edge : edges) {
                    if (!sameHospital("Faulkner",nodeMap.get(edge.getNode1()).getBuilding()) || !sameHospital("Faulkner",nodeMap.get(edge.getNode1()).getBuilding())) {
                        this.nodes.add(node);
                    }
                }
            } else {
            ...
            for (Node startNode: exitNodes) {
                for (Edge edge : startNode.getEdges()) {
                    ...
                    if (!sameHospital(startNode.getBuilding(),endNode.getBuilding())) {
                        double cost = typeScorer.computeCost(from,startNode) + typeScorer.computeCost(endNode,to);
                        if (cost < bestCost) {
                            bestCost = cost;
                        }
                    }
